...
def train_one(num_uavs: int) -> tuple[list, list]:
    ...
    CURRICULUM_FIXED = 0.40   # first 40% of episodes on fixed map

    for episode in range(1, max_episodes + 1):
        if episode / max_episodes < CURRICULUM_FIXED:
            ep_seed = seed        # fixed map — build base policy
        else:
            ep_seed = episode     # random maps — generalise
        env = build_env(num_uavs, seed=ep_seed)
...
def evaluate_agent(num_uavs: int, n_episodes: int = num_eval_episodes) -> dict:
    pth = agent_path(num_uavs)
    env = build_env(num_uavs, seed=seed+100)
    ...
    for ep in range(n_episodes):
        env   = build_env(num_uavs, seed=ep)
        obs   = normalise_obs(env.reset())
        dones = np.zeros(num_uavs, dtype=bool)
